...
class TextProcessor:
    ...
    def filter_sentences(self, s: str) -> str:
        print(f"Original string: {s[:100]}")  # Show first 100 chars of the input
        s = re.sub(r"([©|& Co\.])(\.)", r"\1\2 ", s)
        print(f"After regex: {s[:100]}")  # Show result after regex substitution
        ...
        sentences_filtered = [sentences[i] for i in range(len(sentences)) if i not in indices_to_remove]
